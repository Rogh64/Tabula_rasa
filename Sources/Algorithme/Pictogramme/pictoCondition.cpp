...
PictoCondition::PictoCondition(const QString& label) :
     Pictogramme(), isForeverAlone_( false )
{/*{{{*/
     labels_ << new LabelItem( label, TailleMaxCond , 25, hauteurMax - 10, this );
     labels_.at( 0 )->setAnchorType( AncreItem::Down );

     labels_ << new LabelItem( "Sinon", TailleMaxCond-25, 25, hauteurMax - 10 , this );
...
{/*{{{*/
     ...
     if( !isForeverAlone_ ) {
             ...
             for( int i = 1; i< labels_.size(); i++ ) {
                 painter->drawLine( pos, 0, pos, (hauteurMax - 10) );
...
{/*{{{*/
     if( action == getContextMenuAction("SingleOne") ) {
          ...
          updateDimension();
               ...
               LabelItem* item = new LabelItem( "", TailleMaxCond , 25, (hauteurMax - 10), this );
